...
- **Durable Objects 重载分流**：对高 CPU/高频路径（如 prelogin、connect/token、webauthn、two-factor、notifications、icons、auth-requests）按前缀分流到 `HEAVY_DO`
- **Auth Request 性能优化**：`/api/auth-requests*` 已迁移重路径并优化读取逻辑（过期 cutoff 过滤 + pending 限流），降低轮询场景超时风险
