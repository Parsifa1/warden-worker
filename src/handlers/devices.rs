...
pub(crate) async fn ensure_devices_table(_db: &worker::D1Database) -> Result<(), AppError> {
    Ok(())
}

pub(crate) async fn ensure_auth_requests_table(_db: &worker::D1Database) -> Result<(), AppError> {
    Ok(())
}

pub(crate) async fn ensure_device_management_tables(
    _db: &worker::D1Database,
) -> Result<(), AppError> {
    Ok(())
}
